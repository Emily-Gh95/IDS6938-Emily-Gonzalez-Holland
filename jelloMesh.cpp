	...
	for (int i = 0; i < m_rows + 1; i++)
	{
		for (int j = 0; j < m_cols + 1; j++)
		{
			for (int k = 0; k < m_stacks + 1; k++)
			{
				...
				if ((i<m_rows) && (j<m_cols)) AddShearSpring(GetParticle(g, i + 1, j, k), GetParticle(g, i, j + 1, k));

				if ((i<m_rows) && (k<m_stacks))  AddShearSpring(GetParticle(g, i, j, k), GetParticle(g, i + 1, j, k + 1));
				if ((i<m_rows) && (k<m_stacks))  AddShearSpring(GetParticle(g, i + 1, j, k), GetParticle(g, i, j, k + 1));

				if ((j<m_cols) && (k<m_stacks))  AddShearSpring(GetParticle(g, i, j, k), GetParticle(g, i, j + 1, k + 1));
				if ((j<m_cols) && (k<m_stacks))  AddShearSpring(GetParticle(g, i, j + 1, k), GetParticle(g, i, j, k + 1));


				if ((i<m_rows) && (j<m_cols) && (k<m_stacks)) AddShearSpring(GetParticle(g, i, j, k), GetParticle(g, i + 1, j + 1, k + 1));
				if ((i<m_rows) && (j<m_cols) && (k<m_stacks)) AddShearSpring(GetParticle(g, i, j + 1, k), GetParticle(g, i + 1, j, k + 1));

				if ((i<m_rows) && (j<m_cols) && (k<m_stacks)) AddShearSpring(GetParticle(g, i + 1, j + 1, k), GetParticle(g, i, j, k + 1));
				if ((i<m_rows) && (j<m_cols) && (k<m_stacks)) AddShearSpring(GetParticle(g, i + 1, j, k), GetParticle(g, i, j + 1, k + 1));
...
void JelloMesh::AddStructuralSpring(Particle& p1, Particle& p2)
{
	...
	if ((m_integrationType == RK4) || (m_integrationType == VERLETVELOCITY) || (m_integrationType == LEAPFROG) || (m_integrationType == SYMPLETICEULER))
	{
		m_vsprings.push_back(Spring(STRUCTURAL, p1.index, p2.index, g_structuralKs, g_structuralKd, restLen));
}
	if ((m_integrationType == MIDPOINT) || (m_integrationType == EULER))
	{
		m_vsprings.push_back(Spring(STRUCTURAL, p1.index, p2.index, 2000, 10, restLen));
	}
	if (m_integrationType == VERLET)
	{
		m_vsprings.push_back(Spring(STRUCTURAL, p1.index, p2.index, g_structuralKs, g_structuralKd, restLen));
	}
}
...
void JelloMesh::AddBendSpring(JelloMesh::Particle& p1, JelloMesh::Particle& p2)
{
	...
	if ((m_integrationType == MIDPOINT) || (m_integrationType == EULER))
	{
		m_vsprings.push_back(Spring(BEND, p1.index, p2.index, 1500, 20, restLen));
	}
	if (m_integrationType == VERLET)
	{
		m_vsprings.push_back(Spring(BEND, p1.index, p2.index, g_bendKs, g_bendKd, restLen));
...
void JelloMesh::AddShearSpring(JelloMesh::Particle& p1, JelloMesh::Particle& p2)
{
	...
	if ((m_integrationType == MIDPOINT) || (m_integrationType == EULER))
	{
		m_vsprings.push_back(Spring(SHEAR, p1.index, p2.index, 5000, 25, restLen));
	}
	if (m_integrationType == VERLET)
	{
		m_vsprings.push_back(Spring(SHEAR, p1.index, p2.index, g_shearKs, g_shearKd, restLen));
...
void JelloMesh::Update(double dt, const World& world, const vec3& externalForces)
{
	...
	case VERLET: VerletIntegrate(dt); break;
	case VERLETVELOCITY: VerletVolecityIntegrate(dt); break;
	case LEAPFROG: Leapfrog(dt); break;
	case SYMPLETICEULER: SymplecticEuler(dt); break;
	}
}   
            ...
            for (int k = 0; k < m_stacks+1; k++)
            {
                ...
                for (unsigned int i = 0; i < world.m_shapes.size(); i++)
                {
						...
						SphereIntersection(p, (World::Sphere*) world.m_shapes[i], intersection))
					{
						...
						BoxIntersection(p, (World::Cube*) world.m_shapes[i], intersection))
					{
						if (intersection.m_type == CONTACT)
						{
							m_vcontacts.push_back(intersection);
...
void JelloMesh::ComputeForces(ParticleGrid& grid)
{
    ...
    for(unsigned int i = 0; i < m_vsprings.size(); i++)
    {
		...
		vec3 I = a.velocity - b.velocity;

		double restl = spring.m_restLen;
		double lMag = sqrt(l[0] * l[0] + l[1] * l[1] + l[2] * l[2]);
		double iMag = sqrt(I[0] * I[0] + I[1] * I[1] + I[2] * I[2]);

		vec3 Fa = -(spring.m_Ks *(lMag - restl) + spring.m_Kd *(Dot(l, I) / lMag))*l / lMag;
		vec3 Fb = -Fa;
		a.force = a.force + Fa;
		b.force = b.force + Fb;
    }
...
void JelloMesh::ResolveContacts(ParticleGrid& grid)
{
    for (unsigned int i = 0; i < m_vcontacts.size(); i++)
    {
	   ...
	   vec3 PreF = p.force;
	   double Nr = 0.75;
	   p.force = (PreF - 2 * (Dot(PreF, normal))*normal)*Nr;

	   vec3 PreV = p.velocity;
	   p.velocity = (PreV - 2 * (Dot(PreV, normal))*normal)*Nr;
    }
}
...
void JelloMesh::ResolveCollisions(ParticleGrid& grid)
{
    for(unsigned int i = 0; i < m_vcollisions.size(); i++)
    {
		...
		vec3 PreF = pt.force;
		double Nr = 0.75;
		pt.force = (PreF - 2 * (Dot(PreF, normal))*normal)*Nr;

		vec3 PreV = pt.velocity;
		pt.velocity = (PreV - 2 * (Dot(PreV, normal))*normal)*Nr;
	}
}
...
{
    // TODO
	vec3 partPos = vec3(p.position[0], p.position[1], p.position[2]);
	vec3 P = vec3(p.position[0], 0, p.position[2]);
	vec3 N = vec3(0, 1, 0);
	N = N.Normalize();
	double dis = Dot((partPos - P), N);

	intersection.m_distance = dis;
	intersection.m_normal = vec3(0, 1, 0);
	intersection.m_p = p.index;

	if ((dis<0.03) && (dis >= 0.0))
	{
		p.position = vec3(p.position[0], p.position[1] - dis, p.position[2]);
...
bool JelloMesh::SphereIntersection(Particle& p, World::Sphere* sphere, Intersection& intersection)
{
	...
	vec3 partPos = vec3(p.position[0], p.position[1], p.position[2]);
	vec3 checkVec = partPos - sphereCenter;

	if (checkVec.Length() < (sphereRadius + 3))
	{
		double dis = checkVec.Length() - sphereRadius;
		if ((dis<epislon) && (dis >= 0.0))
		{
			vec3 N = checkVec.Normalize();
			intersection.m_distance = dis;
			intersection.m_normal = N;
			intersection.m_p = p.index;

			p.position = vec3(p.position[0] - dis*N[0], p.position[1] - dis*N[1], p.position[2] - dis*N[2]);
			intersection.m_type = CONTACT;
			return true;
		}
		if ((dis<0.0) && (dis>(-1.0*epislon)))
		{
			vec3 N = checkVec.Normalize();
			intersection.m_distance = dis;
			intersection.m_normal = N;
...
bool JelloMesh::BoxIntersection(Particle& p, World::Cube* cube, Intersection& intersection)
{
	vec3 cubeCenter = cube->pos;
	double X = cube->hx;
	double Y = cube->hy;
	double Z = cube->hz;
	...
	vec3 checkVec = partPos - cubeCenter;

	if (checkVec.Length() <= range)
	{

		vec3 N = vec3(-1, 0, 0);
		double dis = sqrt(checkVec[0] * checkVec[0]) - X;
		if ((dis<0.03) && (dis >= 0.0))
		{
			intersection.m_distance = dis;
			intersection.m_normal = N;
                                 ...
                                 JelloMesh::Intersection& result)
{
    ...
    // TODO
	vec3 partPos = vec3(p.position[0], p.position[1], p.position[2]);
	//center of cylinder calculation 
	vec3 cylinderCenter = 0.5*(cylinderStart + cylinderEnd);
	//range check calculation
	double range = sqrt(cylinderAxis.Length()*0.5*cylinderAxis.Length()*0.5 + cylinderRadius*cylinderRadius);
	vec3 checkVec = partPos - cylinderCenter;

	if (checkVec.Length() <= range)
	{

		vec3 StartToPoint = partPos - cylinderStart;
		double STPlength = StartToPoint.Length();
		double cosAngle = Dot(cylinderAxis, StartToPoint) / (cylinderAxis.Length()*STPlength);
		double sinAngle = sqrt(1 - cosAngle*cosAngle);

		double checkLength = STPlength*sinAngle;
		double dis = checkLength - cylinderRadius;

		if ((dis<0.01) && (dis >= 0.0))
		{
			vec3 axisN = cylinderAxis.Normalize();
			vec3 vProjection = Dot(checkVec, axisN)*axisN;
			vec3 vRejection = checkVec - vProjection;
			vec3 N = vRejection.Normalize();

			result.m_distance = dis;
			result.m_normal = N;
			result.m_p = p.index;
...
void JelloMesh::EulerIntegrate(double dt)
{
	...
	ParticleGrid Grid = m_vparticles;
	calculatePosition(Grid, dt);
	// Step 1
	...
	for (int i = 0; i < m_rows + 1; i++)
	{
		for (int j = 0; j < m_cols + 1; j++)
		{
			for (int k = 0; k < m_stacks + 1; k++)
			{
				...
				Particle& p2 = GetParticle(Grid, i, j, k);
				Particle& k1 = GetParticle(accum1, i, j, k);
